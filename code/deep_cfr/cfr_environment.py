...
try:
    import torch
    from MCCFR_train import StrategyNet as _StrategyNet
    from MCCFR_train import query_strategy_net as _deep_cfr_query
    _DEEP_CFR_AVAILABLE = True
except ImportError:
    _DEEP_CFR_AVAILABLE = False
...
class FixedPokerEnvironment(gym.Env):
    ...
    OBS_DIM = 43 + ACTION_HISTORY_LEN * 7  
    ...
    def __init__(
        ...
        sb: float = 5.0,
        bb: float = 10.0,
        snapshot_dir_a: str = "./snapshotsA",
        snapshot_dir_b: str = "./snapshotsB",
        current_agent: str = "A",
        ...
        self.sb = sb
        self.bb = bb
        self.snapshot_dir_a = snapshot_dir_a
        self.snapshot_dir_b = snapshot_dir_b
        self.current_agent = current_agent.upper()
        ...
        self._deep_cfr_nets   = None   
        self._deep_cfr_device = None
    ...
    def _snapshots_for_dir(self, directory: str) -> list[str]:
        """Return the snapshots from directory"""
        pattern = os.path.join(directory, "snapshot_*.zip")
    ...
    def _available_opponent_snapshots(self) -> list[str]:
        """Return snapshots from the opponents agent's pool """
        
        if self.current_agent == "A":
            return self._snapshots_for_dir(self.snapshot_dir_b)
        else:
            return self._snapshots_for_dir(self.snapshot_dir_a)
    ...
    def load_deep_cfr_model(self, pt_path: str):
        
        if not _DEEP_CFR_AVAILABLE:
            print("  [Deep CFR] torch / MCCFR_train not available, skipping.")
            return
        try:
            ...
            nets = []
            for p in range(2):
                net = _StrategyNet().to(device).eval()
                net.load_state_dict(checkpoint[f"strategy_net_{p}"])
                nets.append(net)
            self._deep_cfr_nets   = nets
            self._deep_cfr_device = device
            print(f"  [Deep CFR] StrategyNets loaded from {os.path.basename(pt_path)}"
                  f" (device: {device})")
        except Exception as e:
            print(f"  [Deep CFR] FAILED to load {pt_path}: {e}")
            self._deep_cfr_nets   = None
            self._deep_cfr_device = None
    ...
    def _opponent_action(self, obs: np.ndarray, player_seat: int) -> int:
        ...
        if self._deep_cfr_nets is not None and _DEEP_CFR_AVAILABLE:
            try:
                return _deep_cfr_query(
                    self._deep_cfr_nets[player_seat],
                    self.state,
                    player_seat,
                    self.action_history,
                    mask,
                    self._deep_cfr_device,
                    deterministic=False,
                )
            except Exception:
                pass
    ...
    def _maybe_run_opponent(self):
        ...
        while (
            ...
            success = self._execute_action(action, player=opponent_seat)
            if success:
    ...
    def step(self, action: int):
        ...
        success = self._execute_action(action, player=self.player_seat)
        if success:
            self.last_actions[self.player_seat] = action
    ...
    def _execute_action(self, action: int, player: int) -> bool:
        """Executes an agents action in pokerkit"""
        try:
            if action == self.FOLD:
                self.state.fold()
